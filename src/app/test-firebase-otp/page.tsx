...
export default function TestFirebaseOTP() {
  ...
  useEffect(() => {
    ...
    const initRecaptcha = async () => {
      try {
        ...
        setMessage('❌ reCAPTCHA initialization failed: ' + error);
      }
    };
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      const success = await verifyFirebaseOTP(otp);
      if (success) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4">
      <div className="max-w-md mx-auto bg-white rounded-lg shadow-md p-6">
        <h1 className="text-2xl font-bold text-center mb-6">Firebase OTP Test</h1>
        
        {/* Current User Status */}
        {user && (
          <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg">
            <p className="text-green-800 text-sm">
              <strong>Logged in as:</strong> {user.phoneNumber}
            </p>
            <button
        ...
        {step === 'success' && (
          <div className="text-center space-y-4">
            ...
            <p className="text-gray-600">
              You have successfully logged in with Firebase OTP.
            </p>
            <button
              onClick={resetTest}
        ...
        <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          ...
          <ul className="text-xs text-blue-700 space-y-1">
            ...
            <li>• You will receive an SMS with a 6-digit OTP</li>
            <li>• Enter the OTP to complete authentication</li>
            <li>• Make sure Firebase Authentication is enabled in your project</li>
          </ul>
        </div>
      </div>
    </div>
